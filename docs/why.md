...
`go-ruby-dotenv/dotenv` reimplements Ruby's `dotenv` in **pure Go, with cgo disabled**. The slice of
...
- any Go program can import `github.com/go-ruby-dotenv/dotenv` directly, with no Ruby runtime;
